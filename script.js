...
// ====================================================================
// 🔴 WAJIB: GANTI DENGAN URL WEB APP MASTER ANDA YANG BARU DI-DEPLOY
...
// ====================================================================
// 2. VARIABEL GLOBAL GABUNGAN (PORTAL & KTA)
// ====================================================================
let KODE_SHARD_AKTIF = ""; 

// Variabel Portal (App 1)
...
// ====================================================================
// 3. FUNGSI PENGHUBUNG (ROUTER & API)
// ====================================================================
async function apiCall(actionName, dataObj = {}) {
    try {
        let payloadBody = { action: actionName, kode_shard: KODE_SHARD_AKTIF, data: dataObj };
        // Kunci API Khusus Portal jika Shard Kosong
        if (!KODE_SHARD_AKTIF) payloadBody.apiKey = "SAPA_KTA_SECRET_2026";
        ...
        if (result.status === "error" || result.success === false) throw new Error(result.message || result.error);
...
// Jembatan untuk fungsi-fungsi lama Aplikasi 1
...
function bukaAplikasiKTA(kodeProvinsi) {
    if(!kodeProvinsi) {
        Swal.fire('Error Database', 'Kode Provinsi belum diset oleh Admin di Master Spreadsheet!', 'error');
        return;
    }
    KODE_SHARD_AKTIF = kodeProvinsi;
    
    // Ubah URL secara visual tanpa refresh
...
document.addEventListener('DOMContentLoaded', () => { 
    ...
    if (dataTersimpan) {
        ...
        callGAS('getAppData').then(res => {
            ...
            Swal.close();
            Swal.fire('Error', 'Gagal memuat data Portal. Pastikan URL Master benar dan Spreadsheet terisi.', 'error');
        });
    }
    setTimeout(cekParameterUrl, 800); 
});

// ====================================================================
// 5. AREA KODE APLIKASI 1 (PORTAL PUSAT)
// ====================================================================

function showLoadingAnim(text) {
    let counter = 5;
    Swal.fire({
        title: text + ' (' + counter + ' detik)', allowOutsideClick: false, showConfirmButton: false,
        didOpen: () => { Swal.showLoading(); const titleEl = Swal.getTitle();
            loadingTimerInterval = setInterval(() => { counter--;
                if (counter > 0) titleEl.textContent = text + ' (' + counter + ' detik)';
                else { titleEl.textContent = 'Sedang memproses...'; clearInterval(loadingTimerInterval); }
            }, 1000);
        }, willClose: () => clearInterval(loadingTimerInterval)
    });
}

function jalankanAplikasiPortal() {
        ...
        setTimeout(() => {
            if(!document.querySelector('.ql-toolbar')) {
                quillPrivasi = new Quill('#editor-privasi', { 
                    theme: 'snow', 
                    modules: { toolbar: [ [{ 'font': [] }, { 'size': ['small', false, 'large', 'huge'] }], [{ 'header': [1, 2, 3, 4, 5, 6, false] }], ['bold', 'italic', 'underline', 'strike'], [{ 'color': [] }, { 'background': [] }], [{ 'script': 'sub'}, { 'script': 'super' }], [{ 'list': 'ordered'}, { 'list': 'bullet' }], [{ 'indent': '-1'}, { 'indent': '+1' }], [{ 'align': [] }], ['blockquote', 'code-block'], ['link', 'image', 'video'], ['clean'] ] }, 
...
function logoutAdminPortal() { localStorage.removeItem('sapaAdminSesi'); switchView('view-public'); document.getElementById('admin-mobile-menu').value = 'menu-dashboard'; }
function handleMobileAdminMenu(val) { if (val === 'logout') { logoutAdminPortal(); } else { showMenu(val); let type = val.replace('menu-', ''); if (['provinsi', 'pengurus', 'slide', 'hukum'].includes(type)) { renderTablePortal(type); } else if (type === 'petunjuk') { loadPetunjukAdmin(); } else if (type === 'pengaturan') { loadPengaturanAdmin(); } } }
...
function showAdminPusatModal() {
    ...
    document.getElementById('modal-header-actions-portal').innerHTML = '<button onclick="closeModalPortal(); switchView(\'view-login\');" class="text-navy bg-gold px-3 py-1 rounded text-sm font-bold hover:bg-yellow-400 transition cursor-pointer shadow"><i class="fa-solid fa-lock"></i> Admin</button>';
...
function initPublicView() {
    try {
        ...
        document.getElementById('teks-pengumuman').innerText = info.pengumuman || 'Selamat datang di SAPA KTA...'; document.getElementById('logo').src = info.logo_url || ''; document.getElementById('nav-title').innerText = info.nama_aplikasi || 'SAPA KTA'; document.getElementById('hero-title').innerText = info.nama_organisasi || 'Selamat Datang'; document.getElementById('hero-subtitle').innerText = info.nama_panjang || '';
        ...
        if(appDataCache.provinsi && appDataCache.provinsi.length > 0) { 
                    ...
                    <div class="prov-card bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden card-hover flex flex-col snap-center shrink-0 w-full min-w-full md:w-[calc(33.333%-1rem)] md:min-w-[calc(33.333%-1rem)] min-h-[250px] box-border" data-nama="${safeNamaProvinsi.toLowerCase()}">
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between space-y-4">
                            <div class="text-sm space-y-3">
                                <div><p class="text-xs text-gray-500 font-bold mb-1">Helpdesk Admin Prov. :</p><p class="font-semibold text-gray-800 flex items-center">👤 ${prov.nama_admin || '-'}</p></div>
                                <div><p class="text-xs text-gray-500 font-bold mb-1">Kontak (HP/WA) :</p><a href="${waUrl}" target="_blank" rel="noopener noreferrer" class="font-semibold text-green-600 flex items-center hover:text-green-700"><i class="fa-brands fa-whatsapp text-green-500 text-xl mr-2"></i> ${prov.hp_admin || '-'}</a></div>
                            </div>
                            <button onclick="bukaAplikasiKTA('${kShard}')" class="block w-full text-center bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 text-white font-bold py-2 rounded shadow-md hover:opacity-90 transition text-sm">Daftar & Login Anggota</button>
                        </div>
                    </div>`;
            }); 
        } else {
            provContainer.innerHTML = "<p class='text-center w-full text-gray-500 py-10'>Data Provinsi Belum Tersedia di Master Database.</p>";
        }
...
function slideProvinsi(direction) { const container = document.getElementById('provinsi-container'); const scrollAmount = window.innerWidth < 768 ? container.clientWidth : (container.clientWidth / 3); container.scrollBy({ left: scrollAmount * direction, behavior: 'smooth' }); }
function filterProvinsi() { const input = document.getElementById('searchInput').value.toLowerCase(); document.querySelectorAll('.prov-card').forEach(card => { if(card.getAttribute('data-nama').includes(input)) { card.style.display = ''; } else { card.style.display = 'none'; } }); }

async function handleLoginPortal(e) { 
    e.preventDefault(); const btn = document.getElementById('btn-login-portal'); btn.innerText = "Memeriksa..."; showLoadingAnim('Memeriksa Kredensial Admin');
    try {
        const res = await callGAS('verifyLogin', { username: document.getElementById('username').value, password: document.getElementById('password').value });
        btn.innerText = "Masuk"; 
        if(res && res.success){ 
            localStorage.setItem('sapaAdminSesi', 'aktif'); switchView('view-admin'); renderDashboardPortal(); 
            if(!document.querySelector('.ql-toolbar')) { quillPrivasi = new Quill('#editor-privasi', { theme: 'snow', modules: { toolbar: [ [{ 'font': [] }, { 'size': ['small', false, 'large', 'huge'] }], [{ 'header': [1, 2, 3, 4, 5, 6, false] }], ['bold', 'italic', 'underline', 'strike'], [{ 'color': [] }, { 'background': [] }], [{ 'script': 'sub'}, { 'script': 'super' }], [{ 'list': 'ordered'}, { 'list': 'bullet' }], [{ 'indent': '-1'}, { 'indent': '+1' }], [{ 'align': [] }], ['blockquote', 'code-block'], ['link', 'image', 'video'], ['clean'] ] }, placeholder: 'Ketik Kebijakan Privasi Anda di sini...' }); }
            Swal.fire({toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, icon: 'success', title: 'Login Berhasil'}); 
        } else { Swal.fire('Gagal Login', res.message || 'Error', 'error'); }
    } catch (err) { btn.innerText = "Masuk"; Swal.fire('Error', err.message, 'error'); }
}

function renderDashboardPortal() {
    if (!appDataCache || !appDataCache.provinsi) return;
    let tProv = appDataCache.provinsi.length; let tDaftar = 0, tKTA = 0, tAktif = 0, tTidakAktif = 0;
    appDataCache.provinsi.forEach(prov => { tDaftar += parseInt(prov.jumlah_daftar) || 0; tKTA += parseInt(prov.jumlah_kta) || 0; tAktif += parseInt(prov.jumlah_aktif) || 0; tTidakAktif += parseInt(prov.jumlah_tidak_aktif) || 0; });
    document.getElementById('dash-prov').innerText = tProv.toLocaleString('id-ID'); document.getElementById('dash-daftar').innerText = tDaftar.toLocaleString('id-ID'); document.getElementById('dash-kta').innerText = tKTA.toLocaleString('id-ID'); document.getElementById('dash-aktif').innerText = tAktif.toLocaleString('id-ID'); document.getElementById('dash-nonaktif').innerText = tTidakAktif.toLocaleString('id-ID');
    const ctx = document.getElementById('dashboardChart'); if (!ctx) return; const context2d = ctx.getContext('2d'); if (dashChart) dashChart.destroy(); 
    if (typeof Chart !== 'undefined') { dashChart = new Chart(context2d, { type: 'bar', data: { labels: ['Mendaftar', 'Punya KTA', 'Aktif', 'Tidak Aktif'], datasets: [{ label: 'Jumlah Anggota', data: [tDaftar, tKTA, tAktif, tTidakAktif], backgroundColor: ['rgba(234, 179, 8, 0.8)', 'rgba(99, 102, 241, 0.8)', 'rgba(34, 197, 94, 0.8)', 'rgba(239, 68, 68, 0.8)'], borderColor: ['rgb(202, 138, 4)', 'rgb(79, 70, 229)', 'rgb(22, 163, 74)', 'rgb(220, 38, 38)'], borderWidth: 1, borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { borderDash: [5, 5] } } } } }); }
}

function showMenu(id) { document.querySelectorAll('.menu-content').forEach(el => el.classList.add('hidden')); document.getElementById(id).classList.remove('hidden'); if (id === 'menu-dashboard') { renderDashboardPortal(); } }
function refreshAdminData(renderType) { callGAS('getAppData').then(res => { if(res.success) { appDataCache = res.appData; localStorage.setItem('sapaAppDataCache', JSON.stringify(res.appData)); if(renderType) renderTablePortal(renderType); } }); }
...
function renderTablePortal(type) {
    try {
        const tbody = document.getElementById('table-'+type); tbody.innerHTML = ''; let data = (type === 'hukum') ? appDataCache.dasarHukum : appDataCache[type]; if(!data) data = [];
        if (type === 'provinsi') { if (adminProvSearch) data = data.filter(item => String(item.nama_provinsi).toLowerCase().includes(adminProvSearch)); const totalPages = Math.ceil(data.length / adminProvPerPage); const start = (adminProvPage - 1) * adminProvPerPage; const pageContainer = document.getElementById('pagination-provinsi'); pageContainer.innerHTML = ''; if (totalPages > 1) { pageContainer.classList.remove('hidden'); for (let i = 1; i <= totalPages; i++) { const btnClass = (i === adminProvPage) ? "bg-navy text-white px-3 py-1 rounded" : "bg-gray-200 text-gray-800 px-3 py-1 rounded hover:bg-gray-300"; pageContainer.innerHTML += '<button onclick="changeProvPage('+i+')" class="'+btnClass+'">'+i+'</button>'; } } else { pageContainer.classList.add('hidden'); } data = data.slice(start, start + adminProvPerPage); }
        if (type === 'pengurus') { if (adminPengurusSearch) data = data.filter(item => String(item.nama).toLowerCase().includes(adminPengurusSearch) || String(item.jabatan).toLowerCase().includes(adminPengurusSearch)); const totalPages = Math.ceil(data.length / adminPengurusPerPage); const start = (adminPengurusPage - 1) * adminPengurusPerPage; const pageContainer = document.getElementById('pagination-pengurus'); pageContainer.innerHTML = ''; if (totalPages > 1) { pageContainer.classList.remove('hidden'); for (let i = 1; i <= totalPages; i++) { const btnClass = (i === adminPengurusPage) ? "bg-navy text-white px-3 py-1 rounded" : "bg-gray-200 text-gray-800 px-3 py-1 rounded hover:bg-gray-300"; pageContainer.innerHTML += '<button onclick="changePengurusPage('+i+')" class="'+btnClass+'">'+i+'</button>'; } } else { pageContainer.classList.add('hidden'); } data = data.slice(start, start + adminPengurusPerPage); }
...
async function saveProvinsi(e) { e.preventDefault(); const data = { id: document.getElementById('p-id').value, nama_provinsi: document.getElementById('p-nama').value, link_pendaftaran: document.getElementById('p-link').value, nama_admin: document.getElementById('p-admin').value, hp_admin: document.getElementById('p-hp').value, jumlah_daftar: document.getElementById('p-df').value, jumlah_aktif: document.getElementById('p-ak').value }; try { showLoadingAnim('Menyimpan Data Provinsi'); await callGAS('saveProvinsi', data); closeModalPortal(); refreshAdminData('provinsi'); Swal.fire({toast:true, position:'top-end', showConfirmButton:false, timer:3000, icon:'success', title:'Data Tersimpan'}); } catch (err) { Swal.fire('Error', err.message, 'error'); } }
async function execSaveSlide(e) { e.preventDefault(); const btn = document.getElementById('btn-s-save'); if(!cropper) return Swal.fire('Peringatan', 'Pilih gambar terlebih dahulu!', 'warning'); btn.disabled = true; showLoadingAnim('Mengunggah Slide'); const base64Data = cropper.getCroppedCanvas({width: 1400, height: 800}).toDataURL('image/jpeg', 0.8); const nama = document.getElementById('s-nama').value; try { const res = await callGAS('prosesSimpanSlide', { data: {id: null, nama_gambar: nama}, base64Data: base64Data }); if(res && res.success){ appDataCache.slide = res.updatedList; renderTablePortal('slide'); closeModalPortal(); Swal.fire('Berhasil', 'Slide Tersimpan', 'success'); } else { Swal.fire('Gagal', res?res.error:'Error', 'error'); btn.disabled = false; } } catch(err) { Swal.fire('Error', err.message, 'error'); btn.disabled = false; } }
async function execSavePengurus(e) { e.preventDefault(); const btn = document.getElementById('btn-pe-save'); const data = { id: document.getElementById('pe-id').value, nama: document.getElementById('pe-nama').value, jabatan: document.getElementById('pe-jabatan').value, foto_url: document.getElementById('pe-url-lama').value }; btn.disabled = true; showLoadingAnim('Menyimpan Data Pengurus'); let base64Data = null; if (cropper) { base64Data = cropper.getCroppedCanvas({width: 600, height: 800}).toDataURL('image/jpeg', 0.8); } try { const res = await callGAS('prosesSimpanPengurus', { data: data, base64Data: base64Data }); if(res && res.success) { appDataCache.pengurus = res.updatedList; renderTablePortal('pengurus'); closeModalPortal(); Swal.fire('Berhasil', 'Pengurus Tersimpan', 'success'); } else { Swal.fire('Gagal', res?res.error:'Error', 'error'); btn.disabled = false; } } catch(err) { Swal.fire('Error', err.message, 'error'); btn.disabled = false; } }
async function execSaveHukum(e) { e.preventDefault(); const btn = document.getElementById('btn-h-save'); const fileInput = document.getElementById('h-file'); const data = { id: document.getElementById('h-id').value, nomor: document.getElementById('h-nomor').value, tentang: document.getElementById('h-tentang').value, file_url: document.getElementById('h-url-lama').value }; btn.disabled = true; showLoadingAnim('Menyimpan Dasar Hukum'); try { if (fileInput.files.length > 0) { const base64 = await getBase64(fileInput.files[0]); const resUpload = await callGAS('uploadFileToDrive', { base64Data: base64, fileName: fileInput.files[0].name }); if(resUpload && resUpload.success) { data.file_url = resUpload.url; await callGAS('saveDasarHukum', data); closeModalPortal(); refreshAdminData('hukum'); Swal.fire('Berhasil', 'Data Tersimpan', 'success'); } else { Swal.fire('Gagal', resUpload?resUpload.error:'Error', 'error'); btn.disabled = false; } } else { await callGAS('saveDasarHukum', data); closeModalPortal(); refreshAdminData('hukum'); Swal.fire('Berhasil', 'Data Tersimpan', 'success'); } } catch(err) { Swal.fire('Error', err.message, 'error'); btn.disabled = false; } }
function delData(funcName, id) { Swal.fire({ title: 'Yakin hapus data ini?', text: "Tindakan ini tidak dapat dibatalkan!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Ya, Hapus!', cancelButtonText: 'Batal' }).then(async (result) => { if (result.isConfirmed) { let type = funcName === 'DasarHukum' ? 'hukum' : funcName.toLowerCase(); showLoadingAnim('Menghapus Data'); try { await callGAS('delete' + funcName, id); refreshAdminData(type); Swal.fire('Terhapus!', 'Data berhasil dihapus.', 'success'); } catch (err) { Swal.fire('Error', err.message, 'error'); } } }); }
...
async function savePengaturanUmum(e) { e.preventDefault(); const btn = document.getElementById('btn-save-pengaturan'); showLoadingAnim('Menyimpan Pengaturan'); const htmlPrivasi = quillPrivasi.root.innerHTML; const data = { pengumuman: document.getElementById('set-pengumuman').value, nama_aplikasi: document.getElementById('set-app').value, nama_organisasi: document.getElementById('set-org').value, nama_panjang: document.getElementById('set-panjang').value, sk_kemenkumham: document.getElementById('set-sk').value, website: document.getElementById('set-web').value, email: document.getElementById('set-email').value, telepon: document.getElementById('set-hp').value, alamat: document.getElementById('set-alamat').value, ig: document.getElementById('set-ig').value, fb: document.getElementById('set-fb').value, x: document.getElementById('set-x').value, yt: document.getElementById('set-yt').value, tg: document.getElementById('set-tg').value, tt: document.getElementById('set-tt').value, kebijakan_privasi: htmlPrivasi }; const fileLogo = document.getElementById('set-logo'); try { if(fileLogo.files.length > 0) { const b64 = await getBase64(fileLogo.files[0]); const resUpload = await callGAS('uploadFileToDrive', { base64Data: b64, fileName: 'Logo_Baru.png' }); if(resUpload && resUpload.success) { data.logo_url = resUpload.url; execSavePengaturan(data, btn); } else { Swal.fire('Gagal', resUpload?resUpload.error:'Error', 'error'); } } else { execSavePengaturan(data, btn); } } catch (err) { Swal.fire('Error', err.message, 'error'); } }
async function execSavePengaturan(data, btn) { try { const res = await callGAS('savePengaturan', data); Swal.fire('Berhasil', res.message || 'Tersimpan!', 'success'); refreshAdminData(null); } catch (err) { Swal.fire('Error', err.message, 'error'); } }
...
async function handleSavePetunjuk(e) { e.preventDefault(); const btn = document.getElementById('btn-save-ptj'); showLoadingAnim('Menyimpan Data Petunjuk'); const fileInput = document.getElementById('ptj-pdf'); const narasi = document.getElementById('ptj-narasi').value; const yt = document.getElementById('ptj-yt').value; try { if (fileInput.files.length > 0) { const base64 = await getBase64(fileInput.files[0]); const resUpload = await callGAS('uploadFileToDrive', { base64Data: base64, fileName: fileInput.files[0].name }); if(resUpload && resUpload.success) { await callGAS('savePetunjuk', { narasi: narasi, ytUrl: yt, pdfUrl: resUpload.url }); Swal.fire('Berhasil','Tersimpan!','success'); refreshAdminData(null); } else { Swal.fire('Gagal', resUpload?resUpload.error:'Error', 'error'); } } else { await callGAS('savePetunjuk', { narasi: narasi, ytUrl: yt, pdfUrl: null }); Swal.fire('Berhasil','Tersimpan!','success'); refreshAdminData(null); } } catch(err) { Swal.fire('Error', err.message, 'error'); } }
async function execSaveAdminPusat(e) { e.preventDefault(); const btn = document.getElementById('btn-save-adm'); const data = { admin_nama: document.getElementById('adm-nama').value, admin_jabatan: document.getElementById('adm-jabatan').value, admin_hp: document.getElementById('adm-hp').value, admin_email: document.getElementById('adm-email').value }; btn.disabled = true; showLoadingAnim('Menyimpan Data Admin'); let base64Data = null; if (cropperAdm) { base64Data = cropperAdm.getCroppedCanvas({width: 600, height: 800}).toDataURL('image/jpeg', 0.8); } try { const res = await callGAS('prosesSimpanAdminPusat', { data: data, base64Data: base64Data }); if(res && res.success) { appDataCache = res.appData; Swal.fire('Berhasil', 'Data Admin Pusat Tersimpan!', 'success'); btn.disabled = false; } else { Swal.fire('Gagal', res?res.error:'Error', 'error'); btn.disabled = false; } } catch (err) { Swal.fire('Error', err.message, 'error'); btn.disabled = false; } }
...
// ====================================================================
// 6. AREA KODE APLIKASI 2 (KTA PROVINSI)
// ====================================================================
...
function jalankanAplikasiKTA() {
    ...
    apiCall('getSettings').then(function(s) {
        ...
        // Fallback render kosong untuk debug
...
function getImageUrl(url) { if(!url || typeof url !== 'string' || url.length < 5) return 'https://via.placeholder.com/150?text=NO+IMG'; if(url.startsWith('data:image')) return url; var id = ""; var m1 = url.match(/id=([^&]+)/); var m2 = url.match(/\/d\/([^/]+)/); if (m1) id = m1[1]; else if (m2) id = m2[1]; if (id) return 'https://lh3.googleusercontent.com/d/' + id; return url; }
...
function handleSelfSave(e){ 
    ...
    var d={ 
        ...
        status_pegawai:val('s_statPeg'), status_sekolah:val('s_statSek'),
...
function generateKTA() {
    ...
    apiCall('getKtaResources', {photoUrl: d.foto, logoUrl: appData.settings.logo}).then(async function(assets) {
        ...
        var verifyUrl = baseUrl + "?id=" + KODE_SHARD_AKTIF + "&verify=" + encodeURIComponent(d.nia_lengkap);

        var qr = new QRious({ value: verifyUrl, size: 250, level: 'H' }); 
        const qrImg = await loadImage(qr.toDataURL());
        
        if (qrImg) { 
            ctx.fillStyle = "white"; ctx.fillRect(9, 277, 96, 96); ctx.drawImage(qrImg, 9, 277, 96, 96); 
            if (logoImg) {
                var qrCenterX = 9 + (96 / 2); var qrCenterY = 277 + (96 / 2); var logoSize = 22; var bgSize = 23;   
                ctx.fillStyle = "white"; ctx.fillRect(qrCenterX - (bgSize/2), qrCenterY - (bgSize/2), bgSize, bgSize); ctx.drawImage(logoImg, qrCenterX - (logoSize/2), qrCenterY - (logoSize/2), logoSize, logoSize);
            }
        }

        var photoX = 25; var photoY = 120; var fixedHeight = 132; var fixedWidth = 99; 
        if (photoImg) { var ratio = photoImg.width / photoImg.height; var newWidth = fixedHeight * ratio; ctx.lineWidth = 5; ctx.strokeStyle = "white"; ctx.strokeRect(photoX, photoY, newWidth, fixedHeight); ctx.drawImage(photoImg, photoX, photoY, newWidth, fixedHeight); 
        } else { ctx.fillStyle = "#cbd5e1"; ctx.fillRect(photoX, photoY, fixedWidth, fixedHeight); ctx.fillStyle = "#64748b"; ctx.font = "12px Arial"; ctx.fillText("No Foto", photoX + 25, photoY + 70); }
...
function showForgotInfo() { 
    Swal.fire({ 
        title: 'Lupa Password?', text: "Masukkan alamat email yang terdaftar pada akun Anda.", input: 'email', inputPlaceholder: 'email_anda@gmail.com', showCancelButton: true, confirmButtonText: '<i class="fa fa-paper-plane"></i> Kirim Password', cancelButtonText: 'Batal', confirmButtonColor: '#4f46e5', showLoaderOnConfirm: true, 
        preConfirm: (email) => { return apiCall('forgotPassword', { email: email }).then(res => { if (res.status !== 'success') { throw new Error(res.message); } return res; }).catch(error => { Swal.showValidationMessage(`${error.message}`); }); }, allowOutsideClick: () => !Swal.isLoading() 
    }).then((result) => { if (result.isConfirmed) { Swal.fire({ icon: 'success', title: 'Terkirim!', text: result.value.message, confirmButtonColor: '#10b981' }); } }); 
}
